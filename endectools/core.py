...
def _derive_keys(password: bytes, salt: bytes) -> tuple[bytes, bytes]:
    """
    Derive two 256-bit keys via Scrypt:
...
def _decrypt_file(src: Path, tmp_arch: Path, pwd: bytes) -> None:
    hdr_size = struct.calcsize(HEADER_FMT)
    total_file = src.stat().st_size
    with src.open("rb") as fin:
        hdr = fin.read(hdr_size)
        magic, salt, nonce, _ = struct.unpack(HEADER_FMT, hdr)
        if magic != MAGIC:
...
def shred_file(path: Path, passes: int = 3, pattern: Literal["rand","zero"] = "rand") -> None:
    ...
    for i in range(1, passes+1):
        with path.open("r+b") as f:
            ...
            remaining = size
            while remaining:
                chunk = min(_compute_chunk(size), remaining)
                buf = (secrets.token_bytes(chunk) if pattern=="rand" else b"\x00"*chunk)
                f.write(buf)
                remaining -= chunk
            f.flush(); os.fsync(f.fileno())
        tqdm.tqdm.write(f"Pass {i}/{passes} complete for {path}")
    path.unlink()
...
def shred_path(src: Path, passes: int = 3, pattern: Literal["rand","zero"] = "rand") -> None:
    if src.is_dir():
        for file in src.rglob("*"):
            if file.is_file(): shred_file(file, passes, pattern)
        for dir_ in sorted(src.rglob("*"), reverse=True):
            if dir_.is_dir(): dir_.rmdir()
        src.rmdir()
    else:
        shred_file(src, passes, pattern)
...
def decrypt_path(src_enc: Path, dst_root: Path, pwd: bytes) -> None:
    ...
    with tmp_zst.open("rb") as fi, tmp_tar.open("wb") as fo, \
         dctx.stream_reader(fi) as reader, \
         tqdm.tqdm(total=size_zst, unit="B", unit_scale=True, unit_divisor=1024,
                   desc="Decompressing", leave=False) as pbar:
        for chunk in iter(lambda: reader.read(_compute_chunk(size_zst)), b""):
            fo.write(chunk); pbar.update(len(chunk))
    tmp_zst.unlink()
    with tarfile.open(tmp_tar, "r:") as tar, \
         tqdm.tqdm(total=len(tar.getmembers()), unit="file", desc="Extracting", leave=False) as pbar:
        for m in tar: tar.extract(m, path=dst_root.parent); pbar.update(1)
    tmp_tar.unlink()